...
- **★ 디자인 정본(현재↔폐기 패턴 대조표 포함)은 `DESIGN-SYSTEM.md`입니다. 작업 전 필독.** 이 저장소엔 디자인이 여러 시대로 겹쳐 있어 "사이트에 맞춰줘"가 옛 구현(점선·코너브래킷·먹색 하드섀도우) 복제로 흐르기 쉽다. `DESIGN-SYSTEM.md` §4로 현재/옛것을 구분할 것.
