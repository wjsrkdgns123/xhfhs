...
| # | 컨셉명 | 핵심 아이디어 |
|---|--------|-------------|
| 1 | 호외(號外) — "오늘의 1면" 로비 | 편집국 마스트헤드·본선 대진표·진영 스코어보드. 신문 1면 전체를 로비 레이아웃에 투영 — eyebrow 제호 띠, 섹션 헤드를 면 머리띠로, 방 카드를 대진 명패로 |
| 2 | 대진표 1면 — THE LOBBY LEDGER | 편집국 스코어보드 리디자인. 전광판(scoreboard) 어휘: 섹션 카운트를 대형 발행 호수(號) 배지로, 진영색 이중 룰로 면(面) 경계를 명확히 |
| 3 | 대진표 1면 — THE FIXTURE FRONT PAGE | 진영색 명패·VS 도장 어휘. 빈 자리를 점선 아닌 진영색 하드오프셋 '빈 명패'로, 한자(對) 워터마크로 '미완성 대진'을 표현 |

### 선정 컨셉 및 이유

**합성안 — 「FRONT PAGE — 호외 1면 스코어보드」**

컨셉 #1의 비전(신문 1면 위계) + #2의 안전 규율(전광판 스코어보드·이중 룰) + #3의 진영색 명패·VS 도장 어휘를, `App.tsx의 lb2-* 스타일·App.tsx 안에 실재하는 마크업만`으로 구현 가능한 범위로 재단한 단일안.

선정 이유: 세 컨셉의 강점(비전·규율·어휘)을 취하되, "실재하지 않는 마크업을 새로 만들지 않고 기존 lb2-* 블록만"이라는 안전 제약 안에서 실현 가능한 최대치를 추구. 과감함의 출처는 시각 구조의 위계 강화지, 마크업·핸들러·로직 변경이 아님.
...
|------|------|
| 커밋 해시 | e67d4f1 |
| 변경 파일 | `src/App.tsx` (lb2-* 인라인 `<style>` 블록 + 관련 CSS 클래스만) |
| 빌드 결과 | `✓ built in 2.86s` (exit 0) |
| lint 결과 | `tsc --noEmit && tsc -p tsconfig.functions.json` 통과 (오류 0건) |

**핵심 변경:**

1. **섹션 헤드 → 신문 '면(面) 머리띠' 승격**
   - 상단: 잉크 더블룰(2px solid + 1px inset box-shadow)로 "제호 띠" 구성
   - 하단: 새 토큰 `--lb2-rule`이 `:has()` 선택자로 eyebrow 진영색을 읽어 LIVE=vermillion / OPEN=gold / REPLAY=celadon 룰을 자동 매핑
   - 방 수 카운트를 '발행 호수(號)' 배지로 승격 — 진영색 배경 tint + 잉크 28% 혼합 글자색으로 WCAG AA 확보

2. **빈 자리 카드 → '빈 명패' 어휘 통일**
   - 점선·코너브래킷 완전 제거 (DESIGN-SYSTEM §4 폐기 어휘 0건)
   - 헤어라인 명패 + 대형 한자(對) 워터마크 + hover 시 celadon 2px 잉크 프레임 + 진영색 하드오프셋
   - 방 카드(lb2-card--live/open/ended)의 hover 하드오프셋 어휘와 완전 통일

### 패널 합의

- 세 컨셉 검토 후 합성안 선정 (rounds: 1)
- 구현 게이트 전부 통과: ① build exit0 ② tsc --noEmit exit0 ③ 새 `var(--lb2-rule)` 정의 존재 확인 ④ 금지 문자열(dashed / 먹색 하드오프셋 / raw hex·rgba) 신규 추가 0건
- 마크업·핸들러·Firestore·라우팅·rm2-*·P0 락 영역 미변경 확인

### 변경 파일 폐기패턴 잔존 grep

```
# dashed / 먹색 하드오프셋 검사 (R13 신규 추가분)
git show e67d4f1 -- src/App.tsx | grep "^+" | grep "dashed\|[0-9]px [0-9]px 0.*ink\|rgba("
→ 0건

# raw 6자리 hex 잔존 검사 (R13 신규 추가분)
git show e67d4f1 -- src/App.tsx | grep "^+" | grep "#[0-9a-fA-F]{6}"
→ 0건

# App.tsx rgba 잔존 (R13 이후)
grep -n "rgba(" src/App.tsx
→ 1건 — rgba(16,38,30,0.86): forest(어두운 초록 무대) 기반 스크림.
  R8B 이후 intentional 예외 분류 유지. R13 신규 추가 0건.
```
...
**수렴.** R13 로비 과감 리디자인 1개 커밋(e67d4f1) 완료.

- 섹션 헤드 '면 머리띠' + `--lb2-rule` 자동 매핑: LIVE/OPEN/REPLAY 3개 섹션이 진영색 룰로 명확히 구분됨.
- 빈 명패 어휘: 점선·코너브래킷 완전 제거, 대진표 명패 어휘로 통일.
- 전 변경이 `App.tsx lb2-*` 블록 안에 격리됨(마크업·로직·rm2-*·외부 파일 미변경).
...
**⚠️ 운영자 육안 확인 강력 권고**: 이번은 과감한 비전 변경(섹션 위계 전면 재정의, 빈 자리 카드 어휘 교체)이므로 브라우저 로비 페이지에서 직접 확인 필요.
- 로비(`/`) 방 목록이 있을 때: LIVE NOW / OPEN / REPLAY 섹션 헤드 상단 더블룰·하단 진영색 룰이 각각 vermillion/gold/celadon로 맞게 표시되는지
- 빈 자리 카드: 헤어라인 명패 + 對 워터마크 + hover 시 celadon 하드오프셋이 보이는지
- 4테마(paper/dusk/dawn/ink) + 다크 전환 시 자동 변환 확인

**R1~R13 총 34개 커밋.** 폐기 어휘 전면 제거, WCAG AA·2.1.1 상향, prefers-reduced-motion 가드, 한글 keep-all, 4테마+다크 토큰 자동전환 전 페이지 달성. 색 역할 체계(vermillion=찬성·gold=성취·ink=중립) + 신문 면 머리띠 위계 전 페이지 정립 완료. **디자인 완성도 루프 수렴 유지.**
...
| ID | 제목 | status | 커밋 해시 | 핵심 변경 | 패널 합의 여부 | 운영자 시각확인 항목 |
|----|------|--------|-----------|-----------|---------------|---------------------|
| P15-emptycard | 로비 빈 상태 카드 개선(찾는 논제가 없는가?) | committed | ed47329 | 로비 빈 상태 카드(`.lb2-empty-stage`)를 자기완결 무대로 리디자인. 카드 안 단일 vermillion 알약 CTA('토론방 만들기') 추가로 막다른 길 해소. 본문을 `var(--font-body)`/ink-soft로 정상화하고 hand는 짧은 악센트 1구절로 절제. 한자(開) 워터마크 inset으로 잘림 방지. eyebrow 자간 한글 keep-all 대응. §4 폐기 패턴(점선/코너브래킷/먹색 하드오프셋) 0건 — 1px hairline+soft shadow+vermillion 칼럼 룰의 정본 어휘만 사용. 게이트 전부 통과: 빌드 exit0, 손댄 파일 금지문자열(dashed/0 var(--color-ink)/raw hex/rgba) 0건, 참조 토큰(--color-line/--color-on-accent/--color-vermillion) 모두 정의 확인(새 토큰 불필요). CTA min-height 44 탭타깃·prefers-reduced-motion·4테마/다크 토큰 자동전환 준수. | 합의 (rounds:0) — 정본 기준 충족, 빈 상태 막다른 길 해소 방향 일치. | 로비(`/`) 방이 하나도 없는 빈 상태에서 — 카드 안에 vermillion '토론방 만들기' 버튼이 표시되는지, 한자(開) 워터마크가 잘리지 않고 보이는지 확인. 4테마·다크 자동전환 확인. |
| P15-footer | 전역 푸터 개선 | committed | ad4149e | 푸터를 페이지 배경 무경계 연장에서 `--color-paper-deep` 단차 패널로 전환해 신문 콜로폰 '발' 블록으로 정착(장식 추가 아닌 위계 컨테인먼트 정교화). 도메인 임프린트 `::after`, eyebrow 트래킹 0.08em 한글 라벨 묶기, 콜로폰 바 ink-soft AA 대비 포함. 토큰/var()만 사용·새 토큰 발명 없음(paper-deep는 paper/dusk/dawn/ink+dark 5컨텍스트 모두 정의되어 자동 전환). keep-all/a11y 보존. scope는 `.site-footer` 내부 한정. 게이트: 빌드 exit0(tsc -b && vite build), footer.css 금지문자열(dashed/0 var(--color-ink)/raw hex/rgba) 0건 grep 확인. | 합의 (rounds:1) — R1: 배경색 없는 초안. R2: paper-deep 단차 패널 추가 후 신문 콜로폰 '발' 블록 위계 완성, 합의. | 모든 페이지 하단 푸터 — 배경이 페이지 본문보다 살짝 진한 paper-deep로 구분되는지 확인. dusk/dawn/ink 테마·다크에서 자동 전환 확인. |

### 패널 합의 요약

- **P15-emptycard**: 로비 빈 상태가 '막다른 길'에서 '자기완결 무대'로 전환. rounds:0 합의.
- **P15-footer**: 푸터가 단순 페이지 연장에서 신문 콜로폰 '발' 패널로 정착. rounds:1 합의.

### 최종 `npm run build` 결과

```
✓ built in 2.88s  (exit 0)
기존 chunk-size 경고만 (index-*.js 901kB) — 신규 오류 없음
npm run lint (tsc --noEmit + tsconfig.functions.json) 통과 (오류 0건)
```
...
```
# dashed / 먹색 하드오프셋 검사 (R15 수정 파일 신규 추가분)
git show ed47329 -- src/components/lobby/LobbyEmptyCTA.tsx src/App.tsx | grep "^+" | grep -E "dashed|[0-9]px [0-9]px 0.*var\(--ink|rgba\(|#[0-9a-fA-F]{6}"
→ 0건

git show ad4149e -- src/footer.css | grep "^+" | grep -E "dashed|[0-9]px [0-9]px 0.*var\(--ink|rgba\(|#[0-9a-fA-F]{6}"
→ 0건

# App.tsx rgba 전체 잔존 (R15 이후)
grep -n "rgba(" src/App.tsx
→ 0건 (R8B 이후 모두 토큰화 완료, forest 스크림도 이전 라운드에서 처리)
```
...
**수렴.** R15 병렬 비판 패널 2개 영역 모두 커밋 완료.

- **P15-emptycard**: 로비 빈 상태 카드에서 '막다른 길' 해소 — vermillion CTA + 한자 워터마크로 자기완결 무대 완성. 폐기 어휘 0건.
- **P15-footer**: 푸터를 신문 콜로폰 '발' 블록으로 확립 — paper-deep 단차 패널 + 도메인 임프린트 + eyebrow 라벨. 새 토큰 불필요.
...
**R1~R15 총 37개 커밋.** 폐기 어휘 전면 제거, WCAG AA·2.1.1 상향, prefers-reduced-motion 가드, 한글 keep-all, 4테마+다크 토큰 자동전환 전 페이지 달성. 로비 빈 상태·푸터 콜로폰 위계 완성. **디자인 완성도 루프 수렴 유지.**
